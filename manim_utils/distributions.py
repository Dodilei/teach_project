...
def random_dist(model, n, h, w, dist_set = [1,1], width_ratio=1, is3d = False):
    ...
    for item1 in items:
        for item2 in items:
            ...
            height_dist = (item1.get_height() + item2.get_height())/(2*dist_set[0])
            #width_dist = (item1.height + item2.height)/((1/width_ratio)*2*dist_set[1])
            width_dist = (item1.get_width() + item2.get_width())/(2*dist_set[1])            
